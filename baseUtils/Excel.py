...
def dictFromExcelSheet(sheet_):
    ...
    for _rowNum in range(sheet_.nrows):
        # 规定 第一个 三个都有值的行为起始行
        if str(sheet_.cell(_rowNum, 2).value) != "" and str(sheet_.cell(_rowNum, 3).value) != "" and str(
                sheet_.cell(_rowNum, 4).value) != "":
            _haveStartBool = True
            # 取得 每一列的名称
            for _colNum in range(4, sheet_.ncols):
                ...
                if _currentLocationName and _currentLocationName != "":
                    ...
                    for _rowNumInside in range(_rowNum + 1, sheet_.nrows):
                        if str(sheet_.cell(_rowNumInside, 2).value) != "" and str(
                                sheet_.cell(_rowNumInside, 3).value) != "" and str(
                                sheet_.cell(_rowNumInside, 4).value) != "":
                            _currentLocationDict[str(sheet_.cell(_rowNumInside, 3).value)] = str(
                                sheet_.cell(_rowNumInside, _colNum).value)
                else:
                    break
            break
    return _sheetDict
